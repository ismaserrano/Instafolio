...
class UsersController extends AppController {
	...
	function oauthCallback() {
		...
		if ($this->InstagramApi->readAccessToken()){
			...
			$exists = $this->User->find('first', array('conditions'=>array('User.instagram_user'=>$user->data->username)));
			if (!$exists){
				$this->User->create();
			...
			//foreach ($this->_checkNextMedia($result->pagination->next_url) as $value){
 ...
 */
	...
	private function _checkNextMedia($url=null){
		...
		if (!is_null($url)){
			...
			$this->_checkNextMedia($mediaNext->pagination->next_url);
		}
