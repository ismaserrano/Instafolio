			...
			default:
				if (!empty($slug) && $slug!=''){
					...
					if (!$exists){
						...
						$this->InstagramApi->saveAccessToken($user['User']['access_token']);
						//$this->set('user', $user);
						$media = $this->InstagramApi->getUserRecent($user['User']['instagram_id']);
						$result = json_decode($media);
						
						//Too much recursive!!!
						//if ($_SERVER['REMOTE_ADDR']=='85.53.50.139'){
						//	$recursive = new UsersController;
						//	foreach ($recursive->checkNextMedia($result->pagination->next_url) as $value){
						//		foreach ($value as $value2){
						//			array_push($result->data, $value2);
						//		}
						//	}
						//}
						$this->set('media', $result);
						
						$personal = $this->InstagramApi->getUser($user['User']['instagram_id']);
